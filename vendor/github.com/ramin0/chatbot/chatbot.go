...
import (
	...
	"github.com/ramin0/chatbot-sample/factory"
	"github.com/ramin0/chatbot-sample/util"
...
type (
	...
	Processor func(session Session, message string, x int) (string, error)
)

func sampleProcessor(session Session, message string, x int) (string, error) {
...
func handleUsernamelogin(w http.ResponseWriter, r *http.Request) {
	...
	_, messageFound := data["username"]
	if !messageFound {
		http.Error(w, "Missing username key in body.", http.StatusBadRequest)
		return
	}

	// Process the received message
	username, err := processor(session, data["username"].(string), 1)
	if err != nil {
...
func handleChat(w http.ResponseWriter, r *http.Request) {
	...
	// Process the received message
	message, err := processor(session, data["message"].(string), 0)
	if err != nil {
		http.Error(w, err.Error(), 422 /* http.StatusUnprocessableEntity */)
		return
	}
...
func Engage(addr string) error {
	...
	mux.HandleFunc("/Asklogin", withLog(handleLogin))
	mux.HandleFunc("/Usernamelogin", withLog(handleUsernamelogin))
